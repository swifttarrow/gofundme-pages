...
async function evaluateAndAwardBadges(userId, options = {}) {
    const [fundraiserStats, donationStats] = await Promise.all([
       ...
       FROM fundraisers
       WHERE organizer_id = $1 AND status = 'active'`, [userId]),
        client_1.db.query(`SELECT
    ...
    const awardedBadges = await client_1.db.transaction(async (client) => {
        const newlyAwarded = [];
        for (const badgeType of eligibleBadgeTypes) {
            const def = exports.BADGE_DEFINITIONS.find((badge) => badge.type === badgeType);
            if (!def)
                continue;
            const result = await client.query(`INSERT INTO badges (user_id, type, label, description, icon, priority)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (user_id, type) DO NOTHING
         RETURNING type`, [userId, def.type, def.label, def.description, def.icon, def.priority]);
            if ((result.rowCount ?? 0) > 0) {
                newlyAwarded.push(def);
                await insertBadgeNotification(client, userId, def, options.sourceEventId);
            }
            else {
                await client.query(`UPDATE badges
           SET label = $3, description = $4, icon = $5, priority = $6
           WHERE user_id = $1 AND type = $2`, [userId, def.type, def.label, def.description, def.icon, def.priority]);
            }
        }
        return newlyAwarded;
    });
...
async function insertBadgeNotification(client, userId, badge, sourceEventId) {
    await client.query(`INSERT INTO notifications (user_id, type, title, body, reason_text, deep_link, source_event_id, dedupe_key)
