...
export async function recommendationsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/api/recommendations", async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const mainResult = await db.query<RecommendationRow>(
      `SELECT r.fundraiser_id, r.score, r.interest_match, r.donation_sim,
    ...
    const explorationResult = await db.query<{
      ...
      `SELECT f.id, f.title, f.cover_image_url, f.goal_cents, f.raised_cents, f.category, f.donor_count
       FROM fundraisers f
       WHERE f.status = 'active'
         AND f.id != ALL($1)
       ORDER BY RANDOM()
       LIMIT $2`,
      [seenIds.length > 0 ? seenIds : ["00000000-0000-0000-0000-000000000000"], explorationSlots]
    );
